...
| **Ветер** | Зональный ветер **Huygens DWE** в `data/titan_wind_huygens.json` (NASA PDS); влияние на воздушно-относительную скорость |
| **Тепло** | **Обшивка**: упрощённая модель входа — нагрев от $\rho$ и $|\mathbf{v}_{\mathrm{rel}}|$ с **переходом разрежённый/континуум**, **сдувом погранслоя** (продукты пиролиза) и **абляционным стоком**; выравнивание к $T_{\mathrm{ext}}$; **отсеки** — обмен с наружной средой, нагрев от обшивки, пока теплозащита на месте; без неё — прямой вклад $q_{\mathrm{dyn}}$ |
...
| `data/titan_wind_huygens.json` | Зональный ветер Huygens DWE (`ZONALWIND.TAB`). Выше верхней точки DWE (~144 км) последнее измеренное зональное значение продлено до 1400 км под высоту входа в сценарии |
