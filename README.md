...
- **Тормозной парашют** — только **после** сброса теплозащиты. **Основной** — при $h < 160\ \mathrm{км}$ и уже раскрытом тормозном. **Сброс парашюта** — при $h < 2\ \mathrm{км}$, только при уже раскрытом основном и после минимального времени «научного спуска» под основным куполом (по умолчанию 2 часа модельного времени, `science_descent_min_s`; при ускорении времени проходит быстрее). Пороги высоты задаются в `config`: `parachute_main_max_deploy_alt_m`, `parachute_jettison_max_alt_m`.
